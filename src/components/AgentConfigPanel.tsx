...
function ToolItem({
  ...
  return (
    <div className='space-y-1.5'>
      <div className={`group py-2 px-3 rounded-md hover:bg-zinc-50 transition-colors ${level > 0 ? 'ml-6' : ''}`}>
        <div className='flex items-start space-x-2.5'>
          ...
          <div className='flex-1 min-w-0'>
            <label htmlFor={tool.name} className='block'>
                ...
                <Button
                  onClick={(e: any) => {
                    ...
                    onEditTool({
                      ...
                      description: getToolDescription(tool.parent_id, tool.name)!
                    })
                  }}
                  variant='ghost'
                  size='sm'
              ...
              <div className='text-xs text-zinc-500 line-clamp-2'>{getToolDescription(tool.parent_id, tool.name)}</div>
            </label>
          </div>
        </div>
      </div>
...
export function AgentConfigPanel() {
  ...
  const {
    ...
    setSelectedTools: setSelectedToolsStore,
    setUpdatePending,
    setPrompt,
    setToolDescription,
    defaultAgent,
    ...
    if (selectedTools.includes(toolName)) {
      ...
      Object.entries(sessionData || {}).forEach(([agent, agentConfig]) => {
        const agentTools = agentConfig.tools
          .filter(tool => tools.includes(generateToolId(tool.parent_id, tool.name)))
          .map(tool => tool.name)
    ...
    <>
      <div className='h-full flex flex-col bg-white border-r border-zinc-200'>
        <div className='p-6 border-b border-zinc-200 bg-white'>
          ...
          <p className='text-zinc-500 text-sm'>Configure your AI agent and available tools</p>
        </div>
        ...
        <div className='flex-1 flex flex-col overflow-hidden'>
          <div className='p-6 pb-0 space-y-6'>
            ...
            <div className='space-y-3'>
              <Label htmlFor='agent-select' className='text-sm text-zinc-700'>
                Select Agent
              </Label>
            ...
            <div className='flex items-center justify-between' style={{ marginLeft: 'calc(var(--spacing) * -6)' }}>
              ...
              <Badge variant='secondary' className='bg-zinc-100 text-zinc-700 border-0'>
                {sessionData?.[selectedAgent]?.selectedTools.length} selected
              </Badge>
            </div>
          </div>
