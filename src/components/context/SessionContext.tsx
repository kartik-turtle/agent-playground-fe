...
export default function SessionContext() {
    ...
    const {userProfile, agentProfile, partnerId, selectedAgent, sessionData, isUpdatePending, partnerDetails, webSocketConnected, sessionId, defaultAgent, setSessionData, setAgents, setUpdatePending, setUserProfile, setAgentProfile, setPartnerId, setSelectedAgent, setThreadChats, setSessionId} = useStore()
    ...
    useEffect(() => {
        if(isUpdatePending && sessionData?.[selectedAgent]) {
            ...
            Object.entries(sessionData).forEach(([agent, value]) => {
                ...
                updatedSessionData[agent] = {'prompt': value.prompt, 'tools': tools, 'thread_id': value.threadId, 'base_thread_id': value.baseThreadId}
            })
    ...
    useEffect(() => {
        if(sessionData?.[selectedAgent] && sessionData[selectedAgent].threadId && sessionData[selectedAgent].baseThreadId && partnerDetails && !sessionData[selectedAgent].threadChats && !sessionData[selectedAgent].baseThreadChats) {
            fetchChats(sessionData[selectedAgent].baseThreadId, sessionData[selectedAgent].threadId).then(([baseThreadChats, threadChats]) => {
                const baseThreadFormattedChats = baseThreadChats.data.messageList.flatMap((messageGroup: any) => messageGroup.messages).map(async (message: any) => await formatChatMessage(message))
                const threadFormattedChats = threadChats.data.messageList.flatMap((messageGroup: any) => messageGroup.messages).map(async (message: any) => await formatChatMessage(message))
            ...
            readUpdateThreadStatus(sessionData[selectedAgent].baseThreadId, sessionData[selectedAgent].threadId, partnerDetails.id)
        }
