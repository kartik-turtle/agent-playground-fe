...
export function ChatPanel({
  ...
  const handleFileDownload = async (file: FileAttachment) => {
    const url = file.url
      ? file.url
      : APIBase.MINTPRO + APIEndpoints.DOWNLOAD_THREAD_FILE + `?id=${file.id}&broker=turtlemint`
      ...
      <div className='border-b bg-white'>
            ...
            <div className='space-y-2'>
              <div className='flex items-center justify-between gap-2'>
                <Label className='text-xs text-zinc-500'>
                  System Prompt {!isPromptEditable && <span className='text-zinc-400'>(Base Truth)</span>}
                </Label>
              ...
              {!isPromptExpanded && !isEditingPrompt && (
                <div className='text-xs text-zinc-600 bg-zinc-50 border border-zinc-200 rounded-lg p-3 line-clamp-2'>
                  {(sessionData
                    ? isPromptEditable
                      ? sessionData[selectedAgent]?.prompt
                      : sessionData[selectedAgent]?.basePrompt
        ...
        <Dialog open={showPromptDialog} onOpenChange={setShowPromptDialog}>
          <DialogContent className='max-w-6xl-important max-h-[80vh]'>
            <DialogHeader>
              <DialogTitle>{version} - System Prompt</DialogTitle>
              <DialogDescription>{!isPromptEditable && 'Base truth prompt (read-only)'}</DialogDescription>
            </DialogHeader>
                    ...
                    className={`${
                      message.role === 'system'
                        ...
                        : `max-w-[80%] rounded-2xl px-4 py-3 ${
                            message.role === 'user' ? 'bg-blue-50 text-zinc-800' : 'bg-zinc-100 text-zinc-800'
                          }`
                    }`}
                  >
                    {/* Message header */}
                    ...
                    {message.role === 'assistant' && (
                      <div className='mb-2'>
                        <span className='text-xs text-zinc-500'>{message.author || 'AI'}</span>
                      </div>
                    )}
                    {message.role === 'system' && (
                      <div className='flex items-center gap-1.5 mb-2'>
                        ...
                        <span className='text-xs text-amber-700'>
                          {message.author === 'user' ? 'You' : message.author} (Internal Note)
                        </span>
                      </div>
                    )}
                    ...
                    {message.files && message.files.length > 0 && (
                      <div className='mb-3 space-y-2'>
                        {message.files.map(file => (
                          ...
                          >
                            <div className={message.role === 'user' ? 'text-blue-700' : 'text-zinc-700'}>
                              {getFileIcon(file.type)}
                            </div>
                            <div className='flex-1 min-w-0'>
                              <div className='text-xs truncate font-medium'>{file.name}</div>
                              <div className={`text-xs ${message.role === 'user' ? 'text-blue-600' : 'text-zinc-500'}`}>
                    ...
                    <div className={`text-xs mt-2 ${message.role === 'user' ? 'text-zinc-400' : 'text-zinc-400'}`}>
                      {message.timestamp}
                    </div>
                  </div>
                </div>
        ...
        <div className='p-4 border-t'>
          ...
          {attachedFiles.length > 0 && (
            <div className='mb-3 flex flex-wrap gap-2'>
              {attachedFiles.map(file => (
                <div key={file.id} className='flex items-center gap-2 bg-zinc-100 rounded-md p-2 pr-1 max-w-[200px]'>
                  ...
                  <Button variant='ghost' size='sm' className='h-6 w-6 p-0' onClick={() => handleRemoveFile(file.id)}>
                    <X className='h-3 w-3' />
                  </Button>
                </div>
              ))}
          ...
          <div className='flex gap-2'>
            ...
            <Button
              ...
              className={isInternalNote ? 'bg-zinc-600 hover:bg-zinc-700' : ''}
            >
            ...
            <Input
              ...
              placeholder={isInternalNote ? 'Type your internal note...' : 'Type your message...'}
              disabled={isLoading}
            />
            <Button
              onClick={handleSend}
