...
const ChatInterface = () => {
  ...
  const handleFileUpload = async (files: FileAttachment[]) => {
    ...
    const newBaseThreadFiles: FileAttachment[] = files.map((file, idx) => ({
      name: file.name,
      size: baseThreadFileData[idx].size,
      type: baseThreadFileData[idx].type,
      url: file.url,
      id: baseThreadFileData[idx].id
    }))
    const threadFilesData: FileAttachment[] = files.map((file, idx) => ({
      name: file.name,
      size: threadFileData[idx].size,
      type: threadFileData[idx].mimeType,
      url: file.url,
      id: threadFileData[idx].id
    }))
