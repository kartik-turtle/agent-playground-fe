...
export function ProfileConfig() {
  ...
  const {
    setPartnerId: setPartnerIdStore,
    setAuthToken: setAuthTokenStore,
    setPartnerDetails,
    setUserProfile,
    setAgentProfile,
    setUpdatePending,
  ...
  useEffect(() => {
    ...
    if (authToken && authToken.trim().length != 0 && partnerId && partnerId.trim().length != 0) {
      const promise = partnerDetailsApi(
        APIMethod.GET,
        APIBase.MINTPRO,
        APIEndpoints.PARTNER_DETAILS_API + partnerId,
        {},
        {},
        { tenant: 'turtlemint', broker: 'turtlemint' }
      )
      promise.then(data => {
        if (!cancelled) {
              ...
              <TabsList className='bg-transparent border-0 p-0 h-auto'>
                ...
                >
                  <div className='flex items-center gap-2'>
                    ...
                    {userProfileError && <AlertCircle className='h-3.5 w-3.5 text-red-500' />}
                  </div>
                </TabsTrigger>
                <TabsTrigger
                  value='agent'
                ...
                >
                  <div className='flex items-center gap-2'>
                    ...
                    {agentProfileError && <AlertCircle className='h-3.5 w-3.5 text-red-500' />}
                  </div>
                </TabsTrigger>
              </TabsList>
            </div>
