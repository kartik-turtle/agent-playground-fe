...
export const useStore = create<StoreState>(set => ({
  ...
  setSelectedTools: (agent, value) =>
    set(state => ({
      sessionData: { ...state.sessionData, [agent]: { ...state.sessionData![agent]!, selectedTools: value } }
    })),
  setPrompt: (agent, value) =>
    set(state => ({
      sessionData: { ...state.sessionData, [agent]: { ...state.sessionData![agent]!, prompt: value } }
    })),
  setToolDescription: (agent, name, value) =>
    set(state => ({
      sessionData: {
        ...state.sessionData,
        [agent]: {
          ...state.sessionData![agent]!,
          toolsDescription: { ...state.sessionData![agent]!.toolsDescription, [name]: value }
        }
      }
    })),
  setThreadIds: (agent, threadId, baseThreadId) =>
    set(state => ({
      sessionData: {
        ...state.sessionData,
        [agent]: { ...state.sessionData![agent]!, threadId: threadId, baseThreadId: baseThreadId }
      }
    })),
  ...
  setThreadChats: (agent, threadChats, baseThreadChats) =>
    set(state => ({
      sessionData: {
        ...state.sessionData,
        [agent]: { ...state.sessionData![agent]!, threadChats: threadChats, baseThreadChats: baseThreadChats }
      }
    })),
  setNewThreadChat: (agent, chat) =>
    set(state => ({
      sessionData: {
        ...state.sessionData,
        [agent]: {
          ...state.sessionData![agent]!,
          threadChats: [...(state.sessionData![agent]!.threadChats || []), chat]
        }
      }
    })),
  setNewBaseThreadChat: (agent, chat) =>
    set(state => ({
      sessionData: {
        ...state.sessionData,
        [agent]: {
          ...state.sessionData![agent]!,
          baseThreadChats: [...(state.sessionData![agent]!.baseThreadChats || []), chat]
        }
      }
    })),
  setSessionData: (agent, data) =>
    set(state => ({
      sessionData: { ...state.sessionData, [agent]: { ...state.sessionData?.[agent], ...(data as AgentState) } }
    })),
  setSessionId: sessionId => set({ sessionId: sessionId }),
  setLoadingV1: (agent, value) =>
    set(state => ({
      sessionData: { ...state.sessionData, [agent]: { ...state.sessionData![agent]!, loadingV1: value } }
    })),
  setLoadingV2: (agent, value) =>
    set(state => ({
      sessionData: { ...state.sessionData, [agent]: { ...state.sessionData![agent]!, loadingV2: value } }
    })),
